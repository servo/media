...
impl BackendInit for FFMPEGBackend {
...
impl Backend for FFMPEGBackend {
    fn create_audiostream(&self) -> MediaStreamId {
        register_stream(Arc::new(Mutex::new(FFMPEGMediaStream {
    ...
    fn create_stream_output(&self) -> Box<dyn MediaOutput> {
        Box::new(FFMPEGMediaOutput)
    }

    fn create_audioinput_stream(&self, _: MediaTrackConstraintSet) -> Option<MediaStreamId> {
        Some(register_stream(Arc::new(Mutex::new(FFMPEGMediaStream {
            id: MediaStreamId::new(),
        }))))
    }
    ...
    fn create_stream_and_socket(
        ...
        let id = register_stream(Arc::new(Mutex::new(FFMPEGMediaStream {
            id: MediaStreamId::new(),
        })));
        (Box::new(FFMPEGSocket), id)
    }
    ...
    fn create_player(
        ...
        Arc::new(Mutex::new(FFMPEGPlayer))
    }
    ...
    fn get_device_monitor(&self) -> Box<dyn MediaDeviceMonitor> {
        Box::new(FFMPEGMediaDeviceMonitor {})
    }
}

impl AudioBackend for FFMPEGBackend {
    type Sink = FFMPEGAudioSink;
...
impl AudioStreamReader for FFMPEGStreamReader {
    fn pull(&self) -> Block {
        Default::default()
    }
    fn start(&self) {}
...
impl WebRtcBackend for FFMPEGBackend {
    type Controller = FFMPEGWebRtcController;
    fn construct_webrtc_controller(
        _: Box<dyn WebRtcSignaller>,
        _: WebRtcController,
    ) -> Self::Controller {
        FFMPEGWebRtcController
    }
}
...
pub struct FFMPEGSocket;

impl MediaSocket for FFMPEGSocket {
...
impl MediaStream for FFMPEGMediaStream {
    fn as_any(&self) -> &dyn Any {
        self
    }
...
impl Drop for FFMPEGMediaStream {
    fn drop(&mut self) {
        unregister_stream(&self.id);
    }
}

pub struct FFMPEGAudioSink;

impl AudioSink for FFMPEGAudioSink {
...
pub struct FFMPEGMediaOutput;
impl MediaOutput for FFMPEGMediaOutput {
    fn add_stream(&mut self, _stream: &MediaStreamId) {}
}

pub struct FFMPEGWebRtcController;

impl WebRtcControllerBackend for FFMPEGWebRtcController {
...
impl MediaInstance for FFMPEGPlayer {
...
struct FFMPEGMediaDeviceMonitor;

impl MediaDeviceMonitor for FFMPEGMediaDeviceMonitor {
